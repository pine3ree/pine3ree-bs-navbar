...
$.fn.p3bsnavbar = function(options) {
    ...
    this.each(function() {
        var $navbar = $(this);
        var $toggles = $navbar.find(BS_SELECTOR_DATA_TOGGLE);
        ...
        $toggles.on(BS_EVENT_SHOW, function(e) {
            ...
            if (clientWidth >= OPT.breakpoint) {
                // adjust first dropdown-menu h-position
                if ($dropdown.hasClass('nav-item') && !$dropdownMenu.hasClass(BS_CLASS_MENU_END)) {
                    adjustNavItemHorizontalPosition($dropdown, $dropdownMenu, clientWidth, isRTL);
                }
            }
        });
        ...
        $toggles.on(BS_EVENT_HIDDEN, function(e) {
            ...
            $toggle.trigger(P3_EVENT_HIDDEN);
        });
        ...
        $toggles.on(BS_EVENT_CLICK_DATA_API, function(e) {
            ...
            if (OPT.hover && OPT.timeout > 0) {
                var timeoutID = $toggle.data(P3_DATA_TIMEOUT_ID);
                if (timeoutID > 0) {
...
function adjustNavItemHorizontalPosition($dropdown, $dropdownMenu, clientWidth, isRTL) {
    const startProperty = isRTL ? 'right' : 'left';
    const endProperty = isRTL ? 'left' : 'right';
    // Add visibility:hidden and display block to set position while keeping it hidden
    $dropdownMenu.css({visibility: 'hidden'}).addClass(BS_CLASS_SHOW);
    if ($dropdownMenu.offset()[startProperty] + $dropdownMenu.outerWidth() > clientWidth) {
        $dropdownMenu.css({
            [startProperty]: 'auto',
            [endProperty]: 0,
        });
        console.log(startProperty, endProperty);
    }
    $dropdownMenu.removeClass(BS_CLASS_SHOW).css({visibility: ''});
}
...
function handleBreakpointCrossing($navbar, breakpoint, initialWidth) {
    var previousWidth = initialWidth || getClientWidth();
...
function activateHover($navbar, breakpoint, timeout) {
    ...
    $dropdowns.on('mouseenter', function(e) {
        ...
        var $toggle = $(this).find(BS_SELECTOR_DATA_TOGGLE).first();
...
//            const toggle = bootstrap.Dropdown.getOrCreateInstance($toggle.get(0));
//            toggle.show();
